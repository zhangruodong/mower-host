...
- **刀片控制没有**。出于安全整套移除了：固件里这条命令没有分支，上位机确认不了刀片状态、也保证不了停刀，这种接口留着比没有更危险。删了什么见 [docs/移除记录.md](docs/移除记录.md)。UI 的「开始割草」只录像和导航。
- **自动回充只有舵机扫描**。`DockController` 是个空壳：看到 AprilTag 就停下，后面的验证/旋转/退桩没写。
- **建图页是占位符**。`_mapping_widget` 恒为 `None`，`map_updated` 信号没人接。
- **`_from_pi/` 那些包不是我写的**（从树莓派镜像里抽出来的现成包），别传 GitHub，见固件仓库 `robot mower/lawn-mower/LICENSING.md`。
